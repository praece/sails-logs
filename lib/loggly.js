...
Loggly.prototype.log = function (level, msg, meta, callback) {
  ...
  var tags = _.clone(meta.tags) || [];
  tags.push(level);

  metaData.message = msg || (meta.error && meta.error.message) || 'No Message';
  metaData.message = _.trim(metaData.message, '\n');

  if (meta.error && meta.error.stack) metaData.stack = meta.error.stack;

  this.client.log(metaData, tags);
};